...
###############################################################################
#%%##### V0: AUC 0.58
adata = sc.read_h5ad(DATA_PATH + "raw/ROSMAP_raw.h5ad") # NOTE here using the version not yet performed log tranformation
...
adata = anndata.AnnData(
    X = np.hstack([adata.X.toarray(), np.zeros((adata.shape[0], rest_genes.shape[0]))]),
    obs = adata_ori.obs, # NOTE, for V1. TODO DEL LATER
    var = pd.concat([adata.var, rest_genes_var])
)
# TODO FOR V1 DEL LATER
