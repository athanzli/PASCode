...
# adata_ad = adata_ad[~adata_ad.obs.index.isin(tormv.index)]
...
# adata_nps = adata_nps[~adata_nps.obs.index.isin(tormv.index)]
...
adata.obs[ad_cond_col[:-1] + '_pac_score'] = adata_ad.obs['pac_score'].loc[adata.obs.index]
...
sc.pp.scale(adata)
sc.tl.pca(adata)
sc.pp.neighbors(adata) # NOTE why removing 19k (1.4k) leads to large diff of leiden clustering -> b/c 
sc.tl.leiden(adata, resolution=5) # NOTE resolution # 7, 8
sc.tl.umap(adata)
adata.obs['leiden'].value_counts()
# adata.write_h5ad(DATA_PATH + 'c02_' + nps + '_overlap_removed19k.h5ad') # NOTE
# adata.write_h5ad(DATA_PATH + 'c02_' + nps + '_overlap_removed19kafterpca.h5ad')
# adata.write_h5ad(DATA_PATH + 'c02_' + nps + '_overlap_removed19kleidenres7.h5ad')
# adata.write_h5ad(DATA_PATH + 'c02_' + nps + '_overlap.h5ad')

#%% ########################## analyse
# adata = sc.read_h5ad(DATA_PATH + 'c02_' + nps + '_overlap.h5ad')
# adata_ori = sc.read_h5ad(DATA_PATH + 'c02_' + nps + '_overlap.h5ad')
# adata = sc.read_h5ad(DATA_PATH + 'c02_' + nps + '_overlap_removed19k.h5ad')
# adata = sc.read_h5ad(DATA_PATH + 'c02_' + nps + '_overlap_removed19kafterpca.h5ad')
# adata = adata[~adata.obs.index.isin(tormv.index)]

subinfo = PASCode.utils.subject_info(
    adata.obs,
    subid_col=subid_col,
    columns=['Sex', 'c02x', 'c92x'])
subinfo['c92x'].value_counts()
sel0 = subinfo[(subinfo['c92x']=='Depression_Mood')].sample(n=98).index
sel1 = subinfo[(subinfo['c92x']=='Control') & (subinfo['Sex']=='Male')].sample(n=28).index
sel2 = subinfo[(subinfo['c92x']=='Control') & (subinfo['Sex']=='Female')].sample(n=70).index
sel = np.concatenate([np.concatenate([sel1, sel2]), sel0])
adata.obs['c92_98v98_mask'] = adata.obs[subid_col].isin(sel)
# subinfo = PASCode.utils.subject_info(
#     adata.obs[adata.obs['c92_98v98_mask']],
#     subid_col=subid_col,
#     columns=['Sex', 'c02x', 'c92x'])
# subinfo['c92x'].value_counts()
...
subinfo = PASCode.utils.subject_info(
    ...
    columns=['Sex', 'c92x'])
subinfo['c92x'].value_counts()
subinfo.groupby(['Sex', 'c92x']).size().unstack()
sel0 = subinfo[(subinfo['c92x']=='Depression_Mood')].sample(n=100).index
sel1 = subinfo[(subinfo['c92x']=='Control') & (subinfo['Sex']=='Male')].sample(n=30).index
sel2 = subinfo[(subinfo['c92x']=='Control') & (subinfo['Sex']=='Female')].sample(n=70).index
sel = np.concatenate([np.concatenate([sel1, sel2]), sel0])
adata.obs['c92_100v100_mask'] = adata.obs[subid_col].isin(sel)
...
for cat in cats:
    ...
    if ((1 not in tb.index) or (1 not in tb.columns)) \
        or (tb.loc[1].sum() < thres or tb[1].sum(0) < thres) \
        or (tb.loc[1, 1] < thres):
        continue
...
print(pd.Series(pvalues).sort_values())
...
#%%
###############################################################################
# venn diagram for particular clusters
###############################################################################
# adata.obs['c02_PAC+'] = 0
...
# adata.obs.loc[adata.obs[nps_pac_col]==1, nps + '_PAC+'] = 1
cat = '7' # TODO
# for cat in np.array([12, 15, 29, 31, 34, 35, 43, 44, 55]).astype(str):
try:
...
handles, labels = handles, labels = plot_umap(
    ...
    class_palette=None,
    text_on_plot=False,
    save_path='./c02_' + nps + '_ovlp_leiden_using_c02_umap.tiff'
    # save_path=None
)
...
plot_pac_umap(
    ...
    save_path='./c02_' + nps + '_ovlp_' + nps + '_PAC_using_c02_umap.tiff',
...
for sctp in sctps:
    ...
    adata.obs['pac_pred'] = PASCode.pac.assign_pac(adata.obs['c92_model_pred_score'], mode='cutoff', cutoff=0.5)
    adata.obs['pac_pred_cat'] = ["pac_"+str(adata.obs.iloc[i]['pac_pred']) for i in range(adata.shape[0])]    #use pac_pred
    ...
    # ranksums(sadata2.X[:,np.where(sadata2.var['gene_name']=='IL18')[0]].toarray().flatten()[sadata2.obs['pac_pred_cat']=='pac_-1.0'],
    #             sadata2.X[:,np.where(sadata2.var['gene_name']=='IL18')[0]].toarray().flatten()[sadata2.obs['pac_pred_cat']=='pac_1.0'])
...
neg_cond = 'Control'
# class_col = 'subclass' # NOTE
class_col = 'subtype' # NOTE
# score_col = 'c92_pac_score'
score_col = 'pac_score'
...
mask = adata.obs['c92_100v100_mask'].values
...
# #%%
# subinfo.to_csv("./c92_subinfo.csv")
# shap_values.to_csv(f"./c92_shap_values_{class_col}.csv")

# feature_importance = np.abs(shap_values).mean().sort_values(ascending=False)
# feature_importance.to_csv(f"./c92_feature_importance_{class_col}.csv")
...
shap.summary_plot(
    ...
    cmap=cmap)
plt.savefig(
    f'./c92_celltype_prioritization_by_SHAP_{class_col}.pdf', 
