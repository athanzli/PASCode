...
# %%
adata = sc.read_h5ad(DATA_PATH + 'c92.h5ad')
adata_pac = sc.read_h5ad(DATA_PATH + 'c92_100v100.h5ad')
...
adata.obs['c92_100v100_mask'] = adata.obs.index.isin(adata_pac.obs.index)
...
# NOTE
adata.write_h5ad(DATA_PATH + 'c92.h5ad')
adata_pac.write_h5ad(DATA_PATH + 'c92_100v100.h5ad')
...
torch.save(model.state_dict(), './trained_models/c92_model.pt')

#%%
model.load_state_dict(torch.load('./trained_models/c92_model.pt'))
...
PASCode.utils.distinct_color_scatterplot(
    x=adata.obsm['model_layer2_umap'][:,0],
    y=adata.obsm['model_layer2_umap'][:,1], 
    label= adata.obs['class'], 
    s=1); plt.show()
PASCode.utils.distinct_color_scatterplot(
    x=adata.obsm['model_layer2_umap'][:,0],
    y=adata.obsm['model_layer2_umap'][:,1], 
    label=adata.obs['subclass'], 
    s=1); plt.show()
PASCode.utils.plot_cell_scores(
    x=adata.obsm['model_layer2_umap'][:,0], 
    y=adata.obsm['model_layer2_umap'][:,1],
    scores=adata.obs['pac_score'].values, 
    s=1); plt.show()
