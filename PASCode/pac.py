...
def run_pac(adata, subid_col, cond_col, pos_cond, neg_cond, 
                methods=['milo','meld','cna','daseq']):
    if 'milo' in methods:
        make_nhoods_prop = 0.05 if adata.shape[0] > 1e5 else 0.1
    ...
    adata.obs['rra_milo_meld_cna_daseq'] = rra(adata, score_cols=['milo','meld','cna', 'daseq'])
    adata.obs['rra_milo_meld_daseq'] = rra(adata, score_cols=['milo','meld','daseq'])
    adata.obs['rra'] = adata.obs['rra_milo_meld_cna_daseq']
    adata.obs['rra_pac'] = assign_pac(adata.obs['rra'].values, mode='cutoff', cutoff=0.5)
    adata.obs['pac'] = adata.obs['rra_pac']
    print(adata.obs['pac'].value_counts())
...
def run_cna(adata, subid_col, cond_col, pos_cond, neg_cond):
    print("\n----------------------------- CNA started ... -----------------------------")
    if 'connectivities' not in adata.obsp:
        print('Computing connectivities...')
        sc.pp.neighbors(adata)
