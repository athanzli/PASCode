...
# mask1 = (nbhd_logfc > -1e-2) & (nbhd_logfc < 1e-2)
# mask0 = nbhd_logfc > 2
# mask2 =  nbhd_logfc < -2
# mask = mask1 | mask2 | mask0
# ndata = ttdata.uns['nhood_adata']

# ndata.obs["graph_color"] = ndata.obs["logFC"]
# ndata.obs.loc[ndata.obs["SpatialFDR"]
#                     > 0.1, "graph_color"] = np.nan
# ndata.obs["abs_logFC"] = abs(ndata.obs["logFC"])
# ndata.obs.loc[ndata.obs["abs_logFC"]
#                     < 0, "graph_color"] = np.nan

# # Plotting order - extreme logFC on top
# ndata.obs.loc[ndata.obs["graph_color"].isna(),
#                     "abs_logFC"] = np.nan
# ordered = ndata.obs.sort_values(
#     'abs_logFC', na_position='first').index
# ndata = ndata[ordered]


# sc.pl.embedding(ndata[mask], "X_milo_graph",
#                 color="graph_color", cmap="RdBu_r",
#                 size=ndata[mask].obs["Nhood_size"]*10,
